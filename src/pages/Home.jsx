...
const templates = [
  {
    ...
    subtitle: 'Check the quality of your course before delivering it to learner...',
    tag: 'Microlearning',
  },
  {
    image: 'https://images.unsplash.com/photo-1503676382389-4809596d5290?auto=format&fit=crop&w=400&q=80',
    title: 'E-learning Accessibility Checklist',
    subtitle: 'Ensure that your e-learning content meets the accessibility guid...',
    tag: 'Microlearning',
  },
  {
        ...
        <Stack direction="row" spacing={3} sx={{ overflowX: 'auto', pb: 1 }}>
          {templates.map((tpl, idx) => (
            // <Grid item xs={12} sm={6} md={4} lg={3} key={idx}>
              ...
              >
                <Box sx={{ height: 100, mb: 2, borderRadius: 2, overflow: 'hidden', background: '#f5f5f5' }}>
                  <img src={tpl.image} alt={tpl.title} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                </Box>
                <Box sx={{ mb: 1 }}>
                  <Box sx={{ bgcolor: '#e3f2fd', color: '#2193b0', fontWeight: 600, fontSize: 12, borderRadius: 1, px: 1, py: 0.2, display: 'inline-block', mb: 0.5 }}>{tpl.tag}</Box>
                </Box>
                <Typography sx={{ fontWeight: 600, fontSize: 15, mb: 0.5 }}>{tpl.title}</Typography>
                <Typography sx={{ color: '#888', fontSize: 13, mb: 0.5 }}>{tpl.subtitle}</Typography>
              </Box>
            // </Grid>
          ))}
        </Stack>
